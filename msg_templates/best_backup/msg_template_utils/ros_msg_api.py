...
# Add to list those packages we wish to generate templates for
# group_names_to_generate = ['visualization_msgs',\
#                            'geometry_msgs',\
#                            'actionlib_msgs',\
#                            'diagnostic_msgs',\
#                            'geometry_msgs',\
...
def get_all_ros_msg_groups():
    ...
    for line in subprocess.check_output(['rosmsg', 'list']).splitlines():
        ...
        #if group_name not in group_names_to_generate: continue
        if group_name not in group_names:
            group_names.append(group_name)
            yield RosMsgGroup(group_name)

def basic_to_python2_type(t):
    """See http://wiki.ros.org/msg#Field_Types"""
    if t == 'bool':
        return 'bool'
    elif 'int8' in t or 'int16' in t or 'int32' in t:
        return 'int'
    elif 'int64' in t:
        return 'long'
    elif 'float' in t:
        ...
        raise TypeError('Did not recognize basic type: %s' % t)
...
class RosMsg(object):
    ...
    def __init__(self, grp, msg):
        ...
        for line in msg_lines:
            ...
            if is_basic:
                # is basic -> convert to python2 type
                type = basic_to_python2_type(type)
                group = ''
            else:
